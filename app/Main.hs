...
--TODO: speed up formula generation... vector index?
...
findRun g f n = evalZ3 $ do
  ...
  ids <- mkVarVec mkFreshIntVar "nodeid" indices
  ...
  -- NOTE: can not be a WITHIN a loop if we are on first or last node
      ...
      case sf of  --and the given subformula flag is valid (depending on kind of subf.)
          ...
          Until 1 f g -> do -- this is the regular until
            ...
            lbl_ij_equiv_to $ mkAnd =<< sequence [
              -- case: outside of loop / straight forward path
                join $ mkImplies <$> (mkEq _0 (bls V.! i)) <*> untilConstraint i (n-1)
              -- case: loop border -> find unique partner, check until there
              , join $ mkImplies <$> (mkLt _0 (bls V.! i)) <*> (mkExistsI [j | j<-indices, j/=i] (\j -> do
                          let (i',j')=(min i j, max i j)
                          mkAnd =<< sequence [mkEq (bls V.! i') (bls V.! j'), untilConstraint i' j']))
              -- case: within loop -> find both borders, check until there
              , join $ mkImplies <$> (mkEq _N1 (bls V.! i)) <*> ifWithinLoop i (\j k -> untilConstraint j k)
              ]
  ...
  st <- solverToString
  liftIO $ putStrLn $ "Build constraints: " ++ showTime start start2
  liftIO $ putStrLn $ "Formula size: " ++ show (length st)
  liftIO $ putStrLn "Searching..."
  fmap snd $ withModel $ \m -> do
    end <- liftIO $ getTime Monotonic
    liftIO $ putStrLn $ "Find model: "++showTime start2 end
...
printRun g f r = forM_ r (\(i,b,c,f,ls) -> do
...
g = readGraph [("p",[1]),("py",[2]),("q",[0,3]),("pq",[4,3]),("y",[5]),("z",[3])]
